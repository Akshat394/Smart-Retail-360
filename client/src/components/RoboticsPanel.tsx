...
const RoboticsPanel: React.FC<Props> = ({ filters }) => {
    ...
    (async () => {
      try {
        const [sensorRes, taskRes, analyticsRes] = await Promise.all([
          fetch('/api/warehouse/sensors').then(r => r.json()),
          fetch('/api/warehouse/tasks').then(r => r.json()),
          fetch('/api/warehouse/robot-analytics').then(r => r.json()),
        ]);
        setSensors({
          temperature: sensorRes.temperature,
          humidity: sensorRes.humidity,
          lastUpdate: sensorRes.lastUpdate,
        });
    ...
    wsRef.current.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'warehouse_update' && msg.data) {
          ...
          setSensors({
            temperature: msg.data.sensors.temperature,
            humidity: msg.data.sensors.humidity,
            lastUpdate: msg.data.sensors.lastUpdate,
          });
        }
  ...
  const handleCompleteTask = async (taskId: number) => {
    try {
      await fetch(`/api/warehouse/tasks/${taskId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'Completed', completedAt: new Date().toISOString() }) });
  ...
  // Helper: assign robots to zones (simulate if no zone)
  const robotsWithZone = robots.map((r, i) => ({
    ...r,
  ...
  const zoneRobotCounts = ZONES.map(z => robotsWithZone.filter(r => r.zone === z.id).length);
    ...
    <div className="bg-gray-900 min-h-screen p-6">
      ...
      <div className="flex gap-6 mb-8">
        <div className="bg-gray-800/80 rounded-xl p-6 flex flex-col items-center border-2 border-cyan-500/40 shadow-xl min-w-[180px]">
          ...
          <div className="text-2xl font-bold text-white">{sensors.temperature.toFixed(1)}°C</div>
          <div className="text-xs text-cyan-300">Temperature</div>
        </div>
        <div className="bg-gray-800/80 rounded-xl p-6 flex flex-col items-center border-2 border-cyan-500/40 shadow-xl min-w-[180px]">
          <Droplets className="w-7 h-7 text-cyan-400 mb-2" />
          <div className="text-2xl font-bold text-white">{sensors.humidity.toFixed(1)}%</div>
        ...
        <div className="bg-gray-800/80 rounded-xl p-6 flex flex-col items-center border-2 border-cyan-500/40 shadow-xl min-w-[180px]">
          ...
          <div className="text-white text-sm">{new Date(sensors.lastUpdate).toLocaleTimeString()}</div>
        </div>
      </div>
